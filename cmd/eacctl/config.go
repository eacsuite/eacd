...
var (
	eacdHomeDir           = eacutil.AppDataDir("eacd", false)
	eacctlHomeDir         = eacutil.AppDataDir("eacctl", false)
	eacwalletHomeDir      = eacutil.AppDataDir("eacwallet", false)
	defaultConfigFile     = filepath.Join(eacctlHomeDir, "eacctl.conf")
	defaultRPCServer      = "localhost"
	defaultRPCCertFile    = filepath.Join(eacdHomeDir, "rpc.cert")
	defaultWalletCertFile = filepath.Join(eacwalletHomeDir, "rpc.cert")
)
...
// config defines the configuration options for eacctl.
//
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(eacctlHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		...
		if preCfg.Wallet {
			serverConfigPath = filepath.Join(eacwalletHomeDir, "eacwallet.conf")
		} else {
			serverConfigPath = filepath.Join(eacdHomeDir, "eacd.conf")
		}
...
// For this it tries to read the config file for the RPC server (either eacd or
// eacwallet), and extract the RPC user and password from it.
