...
import (
	...
	_ "github.com/eacsuite/eacd/database/ffldb"
	"github.com/eacsuite/eacd/mempool"
	"github.com/eacsuite/eacd/peer"
	"github.com/eacsuite/eacutil"
)

const (
	defaultConfigFilename        = "eacd.conf"
	defaultDataDirname           = "data"
	defaultLogLevel              = "info"
	defaultLogDirname            = "logs"
	defaultLogFilename           = "eacd.log"
	defaultMaxPeers              = 125
	...
	sampleConfigFilename         = "sample-eacd.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
)

var (
	defaultHomeDir     = eacutil.AppDataDir("eacd", false)
...
// config defines the configuration options for eacd.
//
...
//
// The above results in eacd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.DisableRPC {
		eacdLog.Infof("RPC service is disabled")
	}
	...
	cfg.minRelayTxFee, err = eacutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]eacutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := eacutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	if configFileError != nil {
		eacdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-eacd.conf to the given destination path,
...
// eacdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func eacdDial(addr net.Addr) (net.Conn, error) {
...
// eacdLookup resolves the IP of the given host using the correct DNS lookup
...
func eacdLookup(host string) ([]net.IP, error) {
