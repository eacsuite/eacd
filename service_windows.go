...
const (
	// svcName is the name of eacd service.
	svcName = "eacdsvc"
...
// logServiceStartOfDay logs information about eacd when the main server has
...
// eacdService houses the main service handler which handles all service
// updates and launching eacdMain.
type eacdService struct{}
...
// long-running eacdMain (which is the real meat of eacd), handles service
// change requests, and notifies the service control manager of changes.
func (s *eacdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start eacdMain in a separate goroutine so the service can start
...
// installService attempts to install the eacd service.  Typically this should
...
// removeService attempts to uninstall the eacd service.  Typically this should
...
// startService attempts to start the eacd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &eacdService{})
	if err != nil {
