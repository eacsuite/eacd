...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	addresses []eacutil.Address, nRequired int, kdb KeyDB) ([]byte, bool) {
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	ScriptClass, []eacutil.Address, int, error) {
...
func mergeScripts(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	pkScript []byte, class ScriptClass, addresses []eacutil.Address,
...
func mergeMultiSig(tx *wire.MsgTx, idx int, addresses []eacutil.Address,
...
sigLoop:
	for _, sig := range possibleSigs {
		...
		hash := calcSignatureHash(pkPops, hashType, tx, idx, false)

		for _, addr := range addresses {
			// All multisig addresses should be pubkey addresses
			// it is an error to call this internal function with
			// bad input.
			pkaddr := addr.(*eacutil.AddressPubKey)
...
type KeyDB interface {
	GetKey(eacutil.Address) (*btcec.PrivateKey, bool, error)
}

// KeyClosure implements KeyDB with a closure.
type KeyClosure func(eacutil.Address) (*btcec.PrivateKey, bool, error)

// GetKey implements KeyDB by returning the result of calling the closure.
func (kc KeyClosure) GetKey(address eacutil.Address) (*btcec.PrivateKey,
	bool, error) {
	return kc(address)
}
...
type ScriptDB interface {
	GetScript(eacutil.Address) ([]byte, error)
}

// ScriptClosure implements ScriptDB with a closure.
type ScriptClosure func(eacutil.Address) ([]byte, error)

// GetScript implements ScriptDB by returning the result of calling the closure.
func (sc ScriptClosure) GetScript(address eacutil.Address) ([]byte, error) {
	return sc(address)
}
