...
func ExamplePayToAddrScript() {
	// Parse the address to send the coins to into a eacutil.Address
	// which is useful to ensure the accuracy of the address and determine
	// the address type.  It is also required for the upcoming call to
	// PayToAddrScript.
	addressStr := "Leqay1p3dWuprsnwUjM2SBbrH9BiphaDUL"
	address, err := eacutil.DecodeAddress(addressStr, &chaincfg.MainNetParams)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleSignTxOutput() {
	...
	pubKeyHash := eacutil.Hash160(pubKey.SerializeCompressed())
	addr, err := eacutil.NewAddressPubKeyHash(pubKeyHash,
	...
	lookupKey := func(a eacutil.Address) (*btcec.PrivateKey, bool, error) {
