...
// newAddressPubKey returns a new eacutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) eacutil.Address {
	addr, err := eacutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new eacutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) eacutil.Address {
	addr, err := eacutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new eacutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) eacutil.Address {
	addr, err := eacutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the eacutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the eacutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := eacutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := eacutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := eacutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := eacutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := eacutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*eacutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*eacutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*eacutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*eacutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*eacutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
