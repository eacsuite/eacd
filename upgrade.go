...
// oldBtcdHomeDir returns the OS specific home directory eacd used prior to
// version 0.3.3.  This has since been replaced with eacutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldBtcdHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "eacd")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".eacd")
	}
...
// location prior to eacd version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to eacd
...
func upgradeDBPaths() error {
	...
	upgradeDBPathNet(filepath.Join(oldDbRoot, "eacd.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "eacd_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "eacd_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to eacd
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		eacdLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old eacd.conf into new location if needed.
		...
		if ohpEmpty {
			...
			eacdLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to eacd as new versions require it.
