...
Bitcoin RPC server that uses a eacd/bitcoin core compatible Bitcoin JSON-RPC
API.  This client has been tested with eacd (https://github.com/eacsuite/eacd),
btcwallet (https://github.com/btcsuite/btcwallet), and
bitcoin core (https://github.com/bitcoin).

In addition to the compatible standard HTTP POST JSON-RPC API, eacd and
...
eacd or btcwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by eacd and
...
All notifications provided by eacd require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by eacd (and
...
Also, it is important to realize that eacd intentionally separates the wallet
functionality into a separate process named btcwallet.  This means if you are
connected to the eacd RPC server directly, only the RPCs which are related to
...
The following full-blown client examples are in the examples directory:
 ...
 - eacdwebsockets
   Connects to a eacd RPC server using TLS-secured websockets, registers for
