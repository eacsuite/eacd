...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *eacutil.Tx, details *btcjson.BlockDetails)
	...
	//
	// Deprecated: Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *eacutil.Tx, details *btcjson.BlockDetails)
	...
	// result of a rescan request, due to how eacd may send various rescan
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount eacutil.Amount)
	...
	// such as btcwallet.
	OnAccountBalance func(account string, balance eacutil.Amount, confirmed bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	case btcjson.BtcdConnectedNtfnMethod:
		...
		if err != nil {
			log.Warnf("Received invalid eacd connected "+
				"notification: %v", err)
			return
		}
...
//
// NOTE: This is a eacd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*eacutil.Tx, error) {
	...
	transactions := make([]*eacutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = eacutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*eacutil.Tx,
	*btcjson.BlockDetails, error) {
	...
	return eacutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := eacutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
// parseBtcdConnectedNtfnParams parses out the connection status of eacd
// and btcwallet from the parameters of a eacdconnected notification.
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance eacutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := eacutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
//
// NOTE: This is a eacd extension and requires a websocket connection.
...
//
// Deprecated: Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []eacutil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
...
//
// Deprecated: Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []eacutil.Address) error {
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []eacutil.Address, outpoints []*wire.OutPoint,
...
// NOTE: This is a eacd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []eacutil.Address,
...
// NOTE: This is a eacd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []eacutil.Address, outPoints []wire.OutPoint) error {
