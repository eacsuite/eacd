...
func addrToKey(addr eacutil.Address) ([addrKeySize]byte, error) {
	...
	case *eacutil.AddressWitnessScriptHash:
		var result [addrKeySize]byte
		...
		// push to 20-bytes.
		copy(result[1:], eacutil.Hash160(addr.ScriptAddress()))
		return result, nil

	case *eacutil.AddressWitnessPubKeyHash:
		var result [addrKeySize]byte
...
type AddrIndex struct {
	...
	txnsByAddr      map[[addrKeySize]byte]map[chainhash.Hash]*eacutil.Tx
...
// the passed map.
func (idx *AddrIndex) indexBlock(data writeIndexData, block *eacutil.Block,
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) ConnectBlock(dbTx database.Tx, block *eacutil.Block,
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) DisconnectBlock(dbTx database.Tx, block *eacutil.Block,
	stxos []blockchain.SpentTxOut) error {
...
func (idx *AddrIndex) TxRegionsForAddress(dbTx database.Tx, addr eacutil.Address,
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) indexUnconfirmedAddresses(pkScript []byte, tx *eacutil.Tx) {
	...
	for _, addr := range addresses {
		...
		if addrIndexEntry == nil {
			addrIndexEntry = make(map[chainhash.Hash]*eacutil.Tx)
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) AddUnconfirmedTx(tx *eacutil.Tx, utxoView *blockchain.UtxoViewpoint) {
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) UnconfirmedTxnsForAddress(addr eacutil.Address) []*eacutil.Tx {
	// Ignore unsupported address types.
	addrKey, err := addrToKey(addr)
	if err != nil {
		return nil
	...
	if txns, exists := idx.txnsByAddr[addrKey]; exists {
		addressTxns := make([]*eacutil.Tx, 0, len(txns))
		for _, tx := range txns {
...
func NewAddrIndex(db database.DB, chainParams *chaincfg.Params) *AddrIndex {
	return &AddrIndex{
		...
		txnsByAddr:  make(map[[addrKeySize]byte]map[chainhash.Hash]*eacutil.Tx),
