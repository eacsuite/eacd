...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: eacutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := eacutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*eacutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, eacutil.NewTx(tx))
	}
...
func additionalCoinbase(amount eacutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee eacutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee eacutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee eacutil.Amount) *wire.MsgTx {
