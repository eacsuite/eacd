...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a eacutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := eacutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a eacutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := eacutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (s *server) TransactionConfirmed(tx *eacutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			eacdLookup, func(addrs []*wire.NetAddress) {
...
// newServer returns a new eacd server configured to listen on addr for the
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	amgr := addrmgr.New(cfg.DataDir, eacdLookup)
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *eacutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
