...
eacd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, eacd supports a hybrid,
...
Configuring eacd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the eacd command
line or in the eacd configuration file.  Typically the Tor proxy address will be
...
By default, eacd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since eacd will act as the server.
...
```text
HiddenServiceDir /var/tor/eacd
HiddenServicePort 9333 127.0.0.1:9333
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring eacd as a Tor hidden service requires
three flags:
...
```bash
$ ./eacd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
eacd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the eacd command line or in the eacd
...
```bash
$ ./eacd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
eacd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./eacd --proxy=127.0.0.1:9050 --torisolation
```
