...
5. [How do I use bootstrap.dat with eacd?](#Importing)
...
### 5. How do I use bootstrap.dat with eacd?

eacd comes with a separate utility named `addblock` which can be used to import
...
1. Stop eacd if it is already running.  This is required since addblock needs to
   access the database used by eacd and it will be locked if eacd is using it.
