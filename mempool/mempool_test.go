...
func (s *fakeChain) FetchUtxoView(tx *eacutil.Tx) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) CalcSequenceLock(tx *eacutil.Tx,
...
// transactions.
func txOutToSpendableOut(tx *eacutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   eacutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int32, numOutputs uint32) (*eacutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee eacutil.Amount,
	signalsReplacement bool) (*eacutil.Tx, error) {
	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput eacutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*eacutil.Tx, error) {
	txChain := make([]*eacutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, eacutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	payPubKeyAddr, err := eacutil.NewAddressPubKey(pubKeyBytes, chainParams)
	if err != nil {
		return nil, nil, err
	}
	...
	harness := poolHarness{
		...
		txPool: New(&Config{
			Policy: Policy{
				...
				MinRelayTxFee:        0, // 1 Satoshi per byte
				MaxTxVersion:         1,
			},
			ChainParams:      chainParams,
...
// mock chain.
func (ctx *testContext) addCoinbaseTx(numOutputs uint32) *eacutil.Tx {
	ctx.t.Helper()
...
func (ctx *testContext) addSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee eacutil.Amount,
	signalsReplacement, confirmed bool) *eacutil.Tx {
...
func testPoolMembership(tc *testContext, tx *eacutil.Tx, inOrphanPool, inTxPool bool) {
	tc.t.Helper()
	txHash := tx.Hash()
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   eacutil.Amount(5000000000),
...
func TestSignalsReplacement(t *testing.T) {
	...
	testCases := []struct {
		name               string
		setup              func(ctx *testContext) *eacutil.Tx
...
func TestConflicts(t *testing.T) {
	...
	testCases := []struct {
		...
		setup func(ctx *testContext) (*eacutil.Tx, []*eacutil.Tx)
	}{
		{
		...
		},
		{
			...
			setup: func(ctx *testContext) (*eacutil.Tx, []*eacutil.Tx) {
				...
				return tx, []*eacutil.Tx{conflict1, conflict2}
			},
		},
		{
			// Create a transaction that would introduce two
			...
			setup: func(ctx *testContext) (*eacutil.Tx, []*eacutil.Tx) {
				...
				return tx, []*eacutil.Tx{parent, child}
			},
		},
	}
...
func TestRBF(t *testing.T) {
	...
	const defaultFee = eacutil.SatoshiPerBitcoin
