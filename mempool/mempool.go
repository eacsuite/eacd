...
type Config struct {
	...
	FetchUtxoView func(*eacutil.Tx) (*blockchain.UtxoViewpoint, error)
	...
	// utxo view.
	CalcSequenceLock func(*eacutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
...
type TxPool struct {
	...
	orphansByPrev map[wire.OutPoint]map[chainhash.Hash]*eacutil.Tx
	outpoints     map[wire.OutPoint]*eacutil.Tx
...
func (mp *TxPool) removeOrphan(tx *eacutil.Tx, removeRedeemers bool) {
...
func (mp *TxPool) RemoveOrphan(tx *eacutil.Tx) {
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *eacutil.Tx, tag Tag) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		if _, exists := mp.orphansByPrev[txIn.PreviousOutPoint]; !exists {
			mp.orphansByPrev[txIn.PreviousOutPoint] =
				make(map[chainhash.Hash]*eacutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *eacutil.Tx, tag Tag) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *eacutil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *eacutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *eacutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint, tx *eacutil.Tx, height int32, fee int64) *TxDesc {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *eacutil.Tx) (bool, error) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) signalsReplacement(tx *eacutil.Tx,
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txAncestors(tx *eacutil.Tx,
	cache map[chainhash.Hash]map[chainhash.Hash]*eacutil.Tx) map[chainhash.Hash]*eacutil.Tx {

	// If a cache was not provided, we'll initialize one now to use for the
	// recursive calls.
	if cache == nil {
		cache = make(map[chainhash.Hash]map[chainhash.Hash]*eacutil.Tx)
	}

	ancestors := make(map[chainhash.Hash]*eacutil.Tx)
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txDescendants(tx *eacutil.Tx,
	cache map[chainhash.Hash]map[chainhash.Hash]*eacutil.Tx) map[chainhash.Hash]*eacutil.Tx {
	...
	descendants := make(map[chainhash.Hash]*eacutil.Tx)
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txConflicts(tx *eacutil.Tx) map[chainhash.Hash]*eacutil.Tx {
	conflicts := make(map[chainhash.Hash]*eacutil.Tx)
	for _, txIn := range tx.MsgTx().TxIn {
		conflict, ok := mp.outpoints[txIn.PreviousOutPoint]
		if !ok {
			continue
...
func (mp *TxPool) CheckSpend(op wire.OutPoint) *eacutil.Tx {
	mp.mtx.RLock()
	txR := mp.outpoints[op]
	mp.mtx.RUnlock()
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *eacutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*eacutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) validateReplacement(tx *eacutil.Tx,
	txFee int64) (map[chainhash.Hash]*eacutil.Tx, error) {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAcceptTransaction(tx *eacutil.Tx, isNew, rateLimit, rejectDupOrphans bool) ([]*chainhash.Hash, *TxDesc, error) {
	txHash := tx.Hash()
	...
	var conflicts map[chainhash.Hash]*eacutil.Tx
	if isReplacement {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *eacutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, *TxDesc, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *eacutil.Tx) []*TxDesc {
	...
	for processList.Len() > 0 {
		...
		processItem := firstElement.(*eacutil.Tx)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *eacutil.Tx, allowOrphan, rateLimit bool, tag Tag) ([]*TxDesc, error) {
...
func (mp *TxPool) RawMempoolVerbose() map[string]*btcjson.GetRawMempoolVerboseResult {
	...
	for _, desc := range mp.pool {
		...
		mpd := &btcjson.GetRawMempoolVerboseResult{
			...
			Fee:              eacutil.Amount(desc.Fee).ToBTC(),
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphansByPrev:  make(map[wire.OutPoint]map[chainhash.Hash]*eacutil.Tx),
		nextExpireScan: time.Now().Add(orphanExpireScanInterval),
		outpoints:      make(map[wire.OutPoint]*eacutil.Tx),
	}
}
