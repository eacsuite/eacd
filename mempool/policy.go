...
const (
	...
	DefaultMinRelayTxFee = eacutil.Amount(0)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee eacutil.Amount) int64 {
	...
	if minFee < 0 || minFee > eacutil.MaxSatoshi {
		minFee = eacutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *eacutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee eacutil.Amount) bool {
...
func checkTransactionStandard(tx *eacutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee eacutil.Amount,
...
func GetTxVirtualSize(tx *eacutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
