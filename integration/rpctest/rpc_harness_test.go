...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt eacutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(eacutil.Amount(5 * eacutil.SatoshiPerBitcoin))
	...
	txid = genSpend(eacutil.Amount(500 * eacutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(eacutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*eacutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, eacutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := eacutil.Amount(250 * eacutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = eacutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := eacutil.Amount(50 * eacutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := eacutil.Amount(numMatureOutputs * 50 * eacutil.SatoshiPerBitcoin)
