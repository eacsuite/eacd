...
import (
	...
	"github.com/eacsuite/eacutil/hdkeychain"
)
...
type memWallet struct {
	...
	addrs map[uint32]eacutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	addrs := make(map[uint32]eacutil.Address)
...
// SetRPCClient saves the passed rpc connection to eacd as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*eacutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          eacutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (eacutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []eacutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt eacutil.Amount,
	feeRate eacutil.Amount, change bool) error {
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := eacutil.Amount(txSize * int(feeRate))
...
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut,
	...
	var outputAmt eacutil.Amount
	for _, output := range outputs {
		outputAmt += eacutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() eacutil.Amount {
	m.RLock()
	defer m.RUnlock()
...
func keyToAddr(key *btcec.PrivateKey, net *chaincfg.Params) (eacutil.Address, error) {
	serializedKey := key.PubKey().SerializeCompressed()
	pubKeyAddr, err := eacutil.NewAddressPubKey(serializedKey, net)
	if err != nil {
		return nil, err
	}
