...
// Harness fully encapsulates an active eacd process to provide a unified
// platform for creating rpc driven integration tests involving eacd. The
// active eacd node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active eacd process is fully
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*eacutil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the eacd node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []eacutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure eacd properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created eacd
...
func (h *Harness) NewAddress() (eacutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() eacutil.Amount {
...
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut,
	feeRate eacutil.Amount) (*chainhash.Hash, error) {
...
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut,
	feeRate eacutil.Amount, change bool) (*wire.MsgTx, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*eacutil.Tx, blockVersion int32,
	blockTime time.Time) (*eacutil.Block, error) {
...
func (h *Harness) GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
	txns []*eacutil.Tx, blockVersion int32, blockTime time.Time,
	mineTo []wire.TxOut) (*eacutil.Block, error) {
	...
	prevBlock := eacutil.NewBlock(mBlock)
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "eacd", "rpctest")
