...
import (
	...
	rpc "github.com/eacsuite/eacd/rpcclient"
	"github.com/eacsuite/eacutil"
)

// nodeConfig contains all the args, and data required to launch a eacd process
...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, error) {
	eacdPath, err := btcdExecutablePath()
	if err != nil {
		eacdPath = "eacd"
	}
...
// arguments returns an array of arguments that be used to launch the eacd
// process.
...
// command returns the exec.Cmd which will be used to start the eacd process.
...
// to the eacd process that is launched via Start().
...
// as the base for the log and data directories for eacd.
...
// start creates a new eacd process, and writes its pid in a file reserved for
...
// stop interrupts the running eacd process process, and waits until it exits
...
// shutdown terminates the running eacd process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := eacutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
