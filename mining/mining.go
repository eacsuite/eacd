...
const (
	...
	MinHighPriority = eacutil.SatoshiPerBitcoin * 144.0 / 250
...
func createCoinbaseTx(params *chaincfg.Params, coinbaseScript []byte, nextBlockHeight int32, addr eacutil.Address) (*eacutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *eacutil.Tx, height int32) error {
...
func logSkippedDeps(tx *eacutil.Tx, deps map[chainhash.Hash]*txPrioItem) {
	if deps == nil {
		return
	}
...
//  |  <= policy.BlockMinSize)          |   |
//   -----------------------------------  --
func (g *BlkTmplGenerator) NewBlockTemplate(payToAddress eacutil.Address) (*BlockTemplate, error) {
	...
	// avoided.
	blockTxns := make([]*eacutil.Tx, 0, len(sourceTxns))
...
mempoolLoop:
	...
	for priorityQueue.Len() > 0 {
		...
		case segwitActive && !witnessIncluded && tx.HasWitness():
			...
			// witness commitment.
			coinbaseCopy := eacutil.NewTx(coinbaseTx.MsgTx().Copy())
...
func (g *BlkTmplGenerator) UpdateExtraNonce(msgBlock *wire.MsgBlock, blockHeight int32, extraNonce uint64) error {
	...
	// TODO(davec): A eacutil.Block should use saved in the state to avoid
